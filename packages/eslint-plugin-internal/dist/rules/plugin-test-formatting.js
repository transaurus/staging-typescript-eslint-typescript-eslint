...
export default createRule({
    ...
    meta: {
        ...
        messages: {
            ...
            noUnnecessaryNoFormat: 'noFormat is unnecessary here. Use the fixer to remove it.',
        ...
        schema: [
            {
                ...
                properties: {
                    formatWithPrettier: {
                        type: 'boolean',
                        description: 'Whether to enforce formatting of code snippets using Prettier.',
                    },
                },
            },
        ],
    ...
    create(context, [{ formatWithPrettier }]) {
        ...
        function getCodeFormatted(code) {
            try {
                return prettier
                    .format(code, {
                    ...prettierConfig,
        ...
        function getCodeFormattedOrReport(code, location) {
        ...
        function checkTemplateLiteral(literal, isErrorTest, isNoFormatTagged = false) {
            ...
            const parentIndent = getExpectedIndentForNode(literal, context.sourceCode.lines);
        ...
        function checkForUnnecessaryNoFormat(text, expr) {
            if (text == null) {
                return;
            }
        ...
        return {
            ...
            ObjectExpression(node) {
                ...
                const type = getContextualType(checker, services.esTreeNodeToTSNodeMap.get(node));
                if (!type) {
                    return;
                }
                ...
                if (/^(TSESLint\.)?InvalidTestCase\b/.test(typeString)) {
                    ...
                    for (const testProp of node.properties) {
                        if (testProp.type === AST_NODE_TYPES.SpreadElement ||
                            testProp.computed ||
