...
export default createRule({
    ...
    meta: {
        ...
        fixable: 'code',
        messages: {
            noRelativePathsToInternalPackages: 'Use absolute paths instead of relative paths to import modules in other internal packages.',
        },
        schema: [],
    },
    defaultOptions: [],
    create(context) {
        return {
            ImportDeclaration(node) {
                ...
                if (importSource.value.startsWith('@typescript-eslint') ||
                ...
                const pathOfFileFromPackagesDir = path.relative(PACKAGES_DIR, context.physicalFilename);
                const packageOfFile = pathOfFileFromPackagesDir.split(path.sep)[0];
                const absolutePathOfImport = path.resolve(path.dirname(context.physicalFilename), importSource.value);
                const pathOfImportFromPackagesDir = path.relative(PACKAGES_DIR, absolutePathOfImport);
                ...
                if (packageOfImport !== packageOfFile) {
                    context.report({
                        ...
                        fix: fixer => {
                            ...
                            // results on windows.
                            const platformIndependentRelativePathOfImportFromPackagesDir = pathOfImportFromPackagesDir.split(path.sep).join('/');
                            return fixer.replaceText(importSource, `'@typescript-eslint/${platformIndependentRelativePathOfImportFromPackagesDir}'`);
                        },
                    });
                }
            },
